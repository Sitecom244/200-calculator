...
const PROFIT_MARGIN_SMALL = 0.79; // For quantity < 51
const PROFIT_MARGIN_MEDIUM = 0.69; // For quantity 51-250
const PROFIT_MARGIN_LARGE = 0.73; // For quantity > 250
...
function calcformCalculate() {
    ...
    const Mat = document.getElementById('field-box-quality');
    const Col = document.getElementById('field-box-color');
    const length = parseLocaleFloat(document.getElementById('field-box-length').value);
    const width = parseLocaleFloat(document.getElementById('field-box-width').value);
    const height = parseLocaleFloat(document.getElementById('field-box-height').value);
    const quantity = Number.parseInt(document.getElementById('field-box-quantity').value, 10);
    const material = parseLocaleFloat(document.getElementById('field-box-quality').value);
    const materialText = Mat.options[Mat.selectedIndex].text;
    const color = document.getElementById('field-box-color').value;
...
function getContentSurplus(dimensions, quantity) {
    ...
    if (contents > CONTENT_THRESHOLD_2) surplus += 1;
...
function getLengthSurplus(dimensions) {
    ...
    if (dimensions.length > LENGTH_THRESHOLD_3) surplus += 0.5;
    return surplus;
}
